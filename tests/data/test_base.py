...
from caproj.data.base import BaseDataOps


class BaseDataOpsIOTests(unittest.TestCase):
    """Tests to ensure caproj.data.BaseDataOps IO ops function properly"""
    ...
    def test_from_file_csv(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            df_read = BaseDataOps.from_file(fp).df
            self.assertEqual(
    ...
    def test_from_file_inputdf_persists(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            fp = os.path.join(tmp, "test.csv")
            self.data.to_csv(fp, index=False)
            df_read = BaseDataOps.from_file(fp, copy_input=True).df_input
            self.assertEqual(
                pd.testing.assert_frame_equal(self.data, df_read), None,
            )
    ...
    def test_from_file_inputdf_not_created(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with self.assertRaises(AttributeError):
                BaseDataOps.from_file(fp).df_input

    def test_from_object_df(self):
        """Ensure dataframe object is read and stored to BaseDataOps class"""
        df_read = BaseDataOps.from_object(self.data).df
        self.assertEqual(
            pd.testing.assert_frame_equal(self.data, df_read), None,
        )

    def test_from_object_class(self):
        """Ensure class.df object is read and stored to BaseDataOps class"""
        Base_object = BaseDataOps.from_object(self.data)
        df_read = BaseDataOps.from_object(Base_object).df
        self.assertEqual(
            pd.testing.assert_frame_equal(self.data, df_read), None,
        )
    ...
    def test_from_object_fail(self):
        ...
        with self.assertRaises(Exception):
            BaseDataOps.from_object(Invalid_object)

    def test_to_file(self):
        """Ensure to_file saves self.df to disk"""
        with tempfile.TemporaryDirectory() as tmp:
            Base = BaseDataOps.from_object(self.data)
    ...
    def test_log_record_count(self):
        ...
        Base_object = BaseDataOps.from_object(self.data)
        with self.assertLogs("BaseDataOps", level="INFO") as logmsg:
            Base_object.log_record_count(id_col="PID")
            self.assertTrue(len(logmsg) > 0)

    def test_log_record_count_init(self):
        """Ensure BaseDataOps init triggers log_record_count"""
        with self.assertLogs("BaseDataOps", level="INFO") as logmsg:
            BaseDataOps(self.data, copy_input=False)
            self.assertTrue(
                "Number of project change records" in "".join(logmsg.output)
            )

    def test_log_record_count_init_fails(self):
        """Ensure BaseDataOps init log_record_count fails elegantly"""
        with self.assertLogs("BaseDataOps", level="INFO") as logmsg:
            Base = BaseDataOps(pd.DataFrame(columns=["a", "b"]), copy_input=False)
...
class BaseDataOpsReadJsonMapDictTests(unittest.TestCase):
    """Tests to ensure ``BaseDataOps`` _read_json method functions properly"""

    def setUp(self):
        """Set up data for tests"""
        # define dict for saving to json
        self.json_dict = self.map_dict = {"a": 1, "b": 2}
        # initialize BaseDataOps class for use in tests
        self.Base = BaseDataOps(pd.DataFrame(columns=["PID"]), copy_input=False)
    ...
    def test_read_json_no_file_exists_log(self):
        ...
        with self.assertLogs("BaseDataOps", level="WARNING") as logmsg:
...
class BaseDataOpsColLintTests(unittest.TestCase):
    """Tests to ensure ``BaseDataOps`` column linting functions properly"""

    def setUp(self):
        """Set up data for tests"""
        self.orig_colnames = ["test1", "test_2", "test 3", "test-4"]
        self.linted_colnames = ["test1", "test_2", "test_3", "test_4"]
        self.Base = BaseDataOps(
            pd.DataFrame(columns=self.orig_colnames), copy_input=False
        )
        self.Base_nolint = BaseDataOps(
...
class BaseDataOpsColNameTests(unittest.TestCase):
    """Tests to ensure ``BaseDataOps`` column renaming function properly"""
...
class BaseDataOpsColDtypeTests(unittest.TestCase):
    """Tests to ensure ``BaseDataOps`` column dtype conversion functions properly"""
...
class BaseDataOpsSortRecordsTests(unittest.TestCase):
    """Tests to ensure ``BaseDataOps.sort_records`` functions properly"""
